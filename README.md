...
- ✅ **设备地址复合框（V1.0.2）**：输入 IP 与选择设备合并为一个框——手输 IP（可带 :端口，缺省 5555）点「连接」或回车，成功后自动选为当前设备；点 ▼ 展开已连设备浮层（**状态灯 + 完整序列号 + 连接方式徽标（USB/TCP/模拟器）+ 状态中文说明**），点选即设为当前设备；选中后框内显示序列号 + 状态灯 + × 清除。设备列表**后台每 3 秒自动刷新 + 窗口聚焦即刷新 + 浮层展开即刷新**，另有手动「刷新设备」按钮
- ✅ **查看设备列表弹窗**：一键查看每台设备的**序列号 / 连接方式（USB / TCP / 模拟器）/ 状态 / 设备型号 / 设备品牌 / 安卓版本**，可直接「选为当前设备」或断开 TCP 设备（设备属性并行查询、单台 4 秒超时，一台异常不拖垮整个列表）
- ✅ **屏幕采集卡（V1.0.1 重组）**：截图与录屏同卡分段切换，同一时刻只显示当前模式控件
  - **📷 截图**：零输入一键保存（自动存到程序目录 `screenshots/`，也可自选目录），PNG 按时间自动命名
  - **🎥 录屏（V1.0.1 新增）**：一键开始 / 停止（对应 `adb shell screenrecord`），录制中红点闪烁 + mm:ss 计时；视频自动回传保存到程序目录 `videos/`（也可自选目录）；单段最长 3 分钟（Android 硬限制），到时自动停止并保存；需 Android 4.4+；与一键日志抓取互斥（共用设备 shell 通道，界面置灰联动 + 后端双重校验）
- ✅ **文件传输卡（V1.0.1 重组）**：pull 拉取 / push 推送同卡分段切换，交互不变（所有命令后台静默执行，**不弹出任何 cmd 窗口**）
- ✅ **应用管理（V1.0.1 布局增强）**：独占主界面右列全高，应用列表可见行数从约 2 行提升到 10 行以上；一键查询设备应用列表，支持包名关键字过滤与**查询范围切换（全部 / 仅第三方应用**，排除系统预装），每个应用右侧直接提供「启动（am start，旧系统自动回退 monkey）/ 强停（am force-stop）/ 清缓存 / 卸载」按钮（危险操作均有二次确认）；**安装 APK（-r 覆盖）移入本卡底部**（查应用→管应用→装应用闭环）
...
3. 所有设备在「设备连接」的地址框 ▼ 浮层中选择（列表自动保持最新）；TCP 设备也可直接输入 IP 回车连接（成功自动选中）；需要查看设备型号/安卓版本等完整信息时点击「查看设备与历史」
...
│   │   │   ├── record.go    # 设备录屏会话（录制→停止→pull 回传→ftyp 校验，V1.0.1）
...
│   │   │   ├── record_test.go         # 单元测试（录屏状态机/停止命令拼装/mp4 校验，V1.0.1）
